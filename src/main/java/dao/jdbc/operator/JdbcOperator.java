...
import dao.core.param.ExpParam;
import dao.core.type.TypeTransformer;
...
public class JdbcOperator {
    private static final Logger logger = LoggerFactory.getLogger(dao.jdbc.operator.JdbcOperator.class);
    ...
    public static void main(String[] args) {
        dao.jdbc.operator.JdbcOperator jdbcOperator = new dao.jdbc.operator.JdbcOperator();
        NestedCondition condition = NestedCondition.build().addParam("xx", 1);
        String t = jdbcOperator.createDeleteSql("t", condition);
        NestedCondition condition1 = NestedCondition.build().addParam("xx", 1);
        condition1.setRawQueryString("delete from xx where xx in 1");
        String t1 = jdbcOperator.createDeleteSql("t", condition1);
        System.out.println("条件：" + t);
        System.out.println("直接指定sql：" + t1);
    }
